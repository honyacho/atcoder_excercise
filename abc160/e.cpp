...
int main(int argc, char const *argv[])
{
  ...
  vecll Rs(A), Gs(B), Ts(C), Zs(X+Y+C);
  REP(i, A) cin >> Rs[i];
  REP(i, B) cin >> Gs[i];
  sort(Rs.begin(), Rs.end(), std::greater<ll>());
  sort(Gs.begin(), Gs.end(), std::greater<ll>());
  REP(i, X) Zs[i] = Rs[i];
  REP(i, Y) Zs[i+X] = Gs[i];
  REP(i, C) cin >> Zs[i+X+Y];
  sort(Zs.begin(), Zs.end(), std::greater<ll>());
  ll res = 0;
  REP(i, X+Y) res += Zs[i];
