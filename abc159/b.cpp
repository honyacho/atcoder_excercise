...
{
  string S; cin >> S;
  bool kaibun1 = true, kaibun2 = true, kaibun3 = true;

  ll i = 0, j = S.size()-1;
  while (i <= j) {
    kaibun1 = kaibun1 && S[i] == S[j];
    i++; j--;
  }
  ...
  j = S.size() - 1;
  while (i <= j) {
    kaibun3 = kaibun3 && S[i] == S[j];
    i++; j--;
  }

  if (kaibun1 && kaibun2 && kaibun3) cout << "Yes" << endl;
  else cout << "No" << endl;
